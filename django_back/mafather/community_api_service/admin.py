...
class CategoryAdmin(admin.ModelAdmin):
    ...
    list_display = ['name', 'post_type', 'color_preview', 'display_order', 'is_active', 'created_at']
    list_filter = ['post_type', 'is_active', 'created_at']
    search_fields = ['name', 'description']
    ordering = ['post_type', 'display_order']
    readonly_fields = ['id', 'created_at', 'updated_at']
    
    fieldsets = (
        (None, {'fields': ('name', 'description', 'post_type')}),
        ('디자인', {'fields': ('color', 'icon')}),
        ('정렬 및 상태', {'fields': ('display_order', 'is_active')}),
...
class PostAdmin(admin.ModelAdmin):
    ...
    def has_delete_permission(self, request, obj=None):
        """삭제 권한 확인"""
        return request.user.has_perm('community_api_service.can_delete_posts')

    def has_change_permission(self, request, obj=None):
        """수정 권한 확인"""
        return request.user.has_perm('community_api_service.can_manage_posts')

    def get_actions(self, request):
        """권한에 따른 액션 필터링"""
        actions = super().get_actions(request)
        if not request.user.has_perm('community_api_service.can_delete_posts'):
